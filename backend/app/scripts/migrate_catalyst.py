...
async def migrate():
    ...
    # asyncpg 不支持多条 SQL 在一个 prepared statement 中执行，
    # 所以需要逐条执行。
    statements = [
        # 建表
        """
        ...
        """,
        # 索引
        "CREATE INDEX IF NOT EXISTS ix_catalyst_date_heat ON news_catalyst_stocks (catalyst_date, heat_score DESC)",
        "CREATE INDEX IF NOT EXISTS ix_catalyst_confirm ON news_catalyst_stocks (catalyst_date, confirm_level)",
        "CREATE INDEX IF NOT EXISTS ix_catalyst_ts_code ON news_catalyst_stocks (ts_code)",
        "CREATE INDEX IF NOT EXISTS ix_catalyst_date ON news_catalyst_stocks (catalyst_date)",
    ]
