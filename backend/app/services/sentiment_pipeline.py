...
【重要修正规则】
- 对于并购/重组/收购类事件，不能简单等同于"极度利好"。收购本身中性偏好，只有在收购标的优质且对价合理时才给 +3 或以上。
- 人事变动（高管增持、收购控制权）通常不超过 +3，除非金额极其巨大且方向明确。
- 如果新闻涉及的行业/板块明显处于下行周期（如大宗商品暴跌、行业景气度恶化），需谨慎给高分，即使个股事件本身偏利好。
