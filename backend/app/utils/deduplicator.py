...
进入去重前，先根据"清洗后标题+正文"长度做路由：
  ...
  【长文本通道】长度 > 150 字 → 四层去重
    ...
    第四层：标题语义去重（智谱 Embedding API），跨批次跨源跨措辞识别同一事件

  【短文本语义通道】长度 ≤ 150 字（金融快讯等）
    调用智谱 Embedding API 获取向量（模型可通过 ZHIPU_EMBEDDING_MODEL 环境变量切换：
    embedding-3 默认 256 维，embedding-2 固定 1024 维），计算余弦相似度，
    仅与过去 30 分钟内的短文本向量对比。
...
# 模型名从环境变量读取，支持热切换：embedding-3（默认，可自定义维度）/ embedding-2（固定1024维）
ZHIPU_EMBEDDING_MODEL = settings.ZHIPU_EMBEDDING_MODEL
# embedding-3 用 256 维节省存储；embedding-2 不支持 dimensions 参数，固定 1024 维
ZHIPU_EMBEDDING_DIMENSIONS = 256 if ZHIPU_EMBEDDING_MODEL == "embedding-3" else 1024
...
async def _call_zhipu_embedding(texts: list[str]) -> list[list[float]] | None:
    ...
    自适应 embedding-3（可自定义维度）和 embedding-2（固定 1024 维）。
    ...
    # embedding-3 支持自定义维度；embedding-2 不支持 dimensions 参数
    if ZHIPU_EMBEDDING_MODEL == "embedding-3":
        payload["dimensions"] = ZHIPU_EMBEDDING_DIMENSIONS
    ...
    try:
        ...
        logger.info(
            "Zhipu %s: %d texts → %d vectors (%d-dim, %d tokens)",
...
class NewsDeduplicator:
    ...
    async def _load_embedding_index(self) -> None:
        ...
        logger.info(
            ...
            len(self._emb_index), ZHIPU_EMBEDDING_MODEL, ZHIPU_EMBEDDING_DIMENSIONS,
        )
