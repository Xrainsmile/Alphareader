...
| 层级 | 技术 | 说明 |
|------|------|------|
| 前端 | uni-app (Vue 3) | 当前以 H5 网页为主，不规划小程序端 |
...
│  ┌────────────────────┐  ┌──────────────┐  ┌─────┐             │
│  │akshare→腾讯K线→DB  │─→│ 4周期ROC加权  │─→│ PG  │             │
│  │(三级降级)  A股日线   │  │ 百分位排名1~99│  │写入 │             │
│  └────────────────────┘  └──────────────┘  └─────┘             │
│                                                                  │
│  sandbox_nav: 周一~五 11:35 & 15:35（模拟仓净值计算，含 ETF 行情更新）│
│  ┌────────────────────────────────┐  ┌─────┐                    │
│  │新浪实时→akshare→DB前复权→交易价 │─→│ NAV │                    │
│  │(四级降级)  持仓不复权实时价      │  │计算 │                    │
│  └────────────────────────────────┘  └─────┘                    │
└──────────────────────────────────────────────────────────────────┘
                             ↓
...
全量历史日线需要 ~5000 只股票 × 252 天，数据量大，采用三级降级：

| 优先级 | 数据源 | 说明 |
|--------|--------|------|
| 1 | **akshare** (`stock_zh_a_hist`) | 默认首选，逐只获取前复权日线，3 次重试 + 代理轮换 |
| 2 | **腾讯财经 K 线 API** (`web.ifzq.gtimg.cn`) | akshare 失败或数据不足 (<100 只) 时自动切换，批量获取前复权日 K 线，间隔 0.3~0.5s |
| 3 | **PostgreSQL 历史数据** (`load_from_db`) | 在线源全部失败时，用数据库已有行情数据计算（数据日期可能滞后） |

- 股票代码列表：优先 akshare `stock_zh_a_spot_em()`，失败则从 `stock_daily_quote` 表提取已有代码
- 腾讯 API 格式：`http://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param={code},day,,,{days},qfq`
- RS Rating 日期回退：若指定日期无数据，自动找最近有数据的交易日
...
**文件：** `backend/app/services/data_fetcher.py`

A 股行情（`stock_zh_a_hist`）不包含场内 ETF，但观察池可能持有 ETF 标的。系统通过以下机制补充 ETF 行情数据：

- **识别规则**：根据代码前缀判断 ETF（沪: `51/56/58`，深: `15/16`）
- **数据来源**：akshare `fund_etf_hist_em`（前复权日线，列名与股票一致）
- **触发时机**：
  - 定时 NAV 计算前（11:35 / 15:35）自动更新
  - 手动触发 `POST /sandbox/nav/compute` 时自动更新
- **写入位置**：复用 `stock_daily_quote` 表（upsert，与股票共用）
- **回溯范围**：默认 30 天（ETF 只需近期数据）
- **容错**：ETF 行情更新失败不阻塞 NAV 计算（catch + warning 日志）

### 模拟仓 NAV 持仓价格 — 四级降级策略

NAV 计算只需少量持仓（~5-15 只）的**当天不复权实时价**，与 RS Rating 的全量历史前复权需求不同，因此使用独立的降级链：

| 优先级 | 数据源 | 说明 |
|--------|--------|------|
| 1 | **新浪财经实时报价** (`hq.sinajs.cn`) | 不依赖 akshare，HTTP 直接获取不复权价，最可靠 |
| 2 | **akshare 逐只补全** (`stock_zh_a_hist(adjust="")`) | 仅补全新浪缺失的代码 |
| 3 | **DB 前复权收盘价** (`stock_daily_quote`) | 从行情表取最近收盘价 |
| 4 | **最近交易价格** (`sandbox_trades`) | 取最后一笔买入/卖出价 |
| 手动 | `market_value_override` 参数 | API 传入手动市值，覆盖所有自动计算 |

**为何不能与 RS Rating 共用数据源：**
- RS Rating 需要全市场 ~5000 只的历史前复权日线 → 新浪实时接口没有历史数据
- NAV 需要少量持仓的当天不复权价 → 腾讯 K 线 API 只有收盘日线，无盘中实时价
- 两者在数据量、时间维度、复权类型、实时性上都不同，降级策略互补而非重复
...
| `sandbox_nav` | 周一至周五 11:35, 15:35 | 模拟仓净值计算（午盘+尾盘），计算前自动更新 ETF 行情 |
...
| `backend/app/services/data_fetcher.py` | **A 股+ETF 数据**：akshare/腾讯K线/新浪实时 三源降级 + PG 缓存（含 ETF 行情补充） |
